...
if __name__ == '__main__':
    ...
    syscall_names = list(data_dict[scenario]['Test']['Normal and Attack']['protocol_distribution'].keys())
    ...
    for sub_set in data_dict[scenario].keys():
        for run_type in data_dict[scenario][sub_set].keys():
            if args.no_idle and run_type == 'Idle':
                ...
                for syscall in data_dict[scenario][sub_set][run_type]['protocol_distribution'].keys():
                    total_syscall_count += data_dict[scenario][sub_set][run_type]['protocol_distribution'][syscall]
    ...
    plt.suptitle(f'{scenario} Protocol Comparison')
    plt.xlabel('Protocol')
