...
class Recorder:
    ...
    def start_recording(self):
        ...
        print(
            'executing: sysdig -w {outputPath} container.name={containername}'.format(containername=self.container.name,
                                                                                      outputPath=self.outputHandler.getScapPath()))
        args = LIDArgparser().parse_args()
        io_buffer_length = args.io_buffer_length
        self.child = pexpect.spawn('sysdig -w {outputPath} -s {ioBufferLength} container.name={containername}'.format(
            containername=self.container.name, outputPath=self.outputHandler.getScapPath(),
            ioBufferLength=io_buffer_length))
