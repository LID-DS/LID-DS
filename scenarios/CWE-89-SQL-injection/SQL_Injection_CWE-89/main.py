...
class SQLInjection(Scenario):
    ...
    def __init__(self, image_name, port_mapping={}, warmup_time=60, recording_time=300, behaviours=[], **kwargs):
        super().__init__(image_name, port_mapping, warmup_time, recording_time, behaviours, **kwargs)
    ...
    def exploit(self, container):
        ...
        try:
            ...
            self.attack_container = client.containers.run(imagename_attacker, detach=True, name="attacker_1337",
...
class SQLBehaviour(Behaviour):
    ...
    def do_normal(self):
        ...
        try:
            client = docker.from_env()
            self.container = client.containers.run(imagename_client, detach=True, name=self.user, stdin_open=True,
                                                   tty=True)
            print('start container for user: ' + self.user)
            ...
            print('start normal behaviour on client ' + self.user + ' with command: ')
            print(command)
        except Exception as error:
            print(error)
...
for user in logins.keys():
    behaviours.append(SQLBehaviour(warmup_time, recording_time, user, logins[user]))
