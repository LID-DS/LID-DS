...
from algorithms.features.impl import ngram_minus_one
...
if __name__ == '__main__':
    ...
    ohe = OneHotEncoding(RepetitionRemover(int_e,True))

    ngram = Ngram([ohe],True,n)    
    dgram = Dgram([int_e],True)
    c = Concat([dgram, ReturnValue()])
    stide = Stide(c)
    mmstide = MinMaxScaling(stide)
    som = Som(ngram)
    mmsom = MinMaxScaling(som)
    max = Minimum([mmstide,mmsom])
    ...
    ###################
    # the IDS
    generate_and_write_alarms = False
    ids = IDS(data_loader=dataloader,
              resulting_building_block=last_bb,
