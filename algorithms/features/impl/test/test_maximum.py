...
def test_maximum():
    ...
    tid = ThreadID()
    max = StreamMaximum(feature=tid, thread_aware=False, window_length=4)

    assert max._calculate(syscall_14) == 1  # 1
    assert max._calculate(syscall_15) == 2  # 2
    assert max._calculate(syscall_16) == 3  # 3
    assert max._calculate(syscall_17) == 4  # 4
    assert max._calculate(syscall_18) == 5  # 5
    assert max._calculate(syscall_19) == 5  # 1
    assert max._calculate(syscall_20) == 5  # 2
    assert max._calculate(syscall_21) == 5  # 3
    assert max._calculate(syscall_22) == 4  # 4
