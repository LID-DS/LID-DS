...
class HoppingNgram(BuildingBlock):
    """
    calculate ngram form a stream of system call features, only return the ngram each X events, the hopping interval
    """

    def __init__(self, feature_list: list, thread_aware: bool, ngram_length: int, hopping_interval: int):
        """
        feature_list: list of features the ngram should use
        thread_aware: True or False
        ngram_length: length of the ngram
        hopping_interval: hopping interval, 1=sliding window, 2=skip each other event, ... ,n=tumbling window
        """
        super().__init__()
        self._ngram_buffer = {}
        self._thread_aware = thread_aware
        self._ngram_length = ngram_length
        self._hopping_interval = hopping_interval
        self._hopping_count = 0
    ...
    def _calculate(self, syscall: Syscall):
        ...
        # call get_result on each dependency to build the current event vector
        # add the current event vector to the ngram if all dependencies are not None
        for feature in self._dependency_list:
            result = feature.get_result(syscall)
            if result is None:
                all_dependencies_are_not_none = False
            else:
                HoppingNgram._concat(result, dependencies)
        ...
        if all_dependencies_are_not_none:
            ...
            # return the ngram if its complete and hopping_count >= hopping_interval
            if len(self._ngram_buffer[thread_id]) == self._deque_length:
                if self._hopping_count >= self._hopping_interval:
                    self._hopping_count = 0
                    return tuple(self._ngram_buffer[thread_id])
                else:
                    self._hopping_count += 1
        return None
