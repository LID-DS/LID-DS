...
class DataPreprocessor:
    ...
    def _prepare_and_fit_building_blocks(self):
        ...
        calls train on, val on and fit for each building block
        on the training data in the order given by the building block manager
        """
        num_generations = len(
            self._building_block_manager.building_block_generations)
        for current_generation in range(0, num_generations):
            ...
            if not self._fit_needed(
                    self._building_block_manager.building_block_generations[current_generation]):
                pass
            else:
                for current_bb in tqdm(
                        self._building_block_manager.building_block_generations[current_generation],
